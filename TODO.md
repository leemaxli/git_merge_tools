...
Design: (a) engine preflight skips a dirty/in-progress TARGET worktree (abort only on dirty MAIN);
(b) engine gains `-ExcludeBranches` so gitsync can exclude origin-pull-unsafe (e.g. conflicting-divergence)
targets; (c) gitsync pull phase skips pull-unsafe TARGETs (abort on unsafe MAIN) and passes them to

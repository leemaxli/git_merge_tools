...
功能完整、测试充分(已知缺陷全部修复;两运行时 103 项测试全绿)。**结构性重构主体已完成**:已抽出
`Core.psm1`(git 原语)与 `Merge.psm1`(事务引擎),三条命令成为同一引擎上的薄壳并去除了命令间耦合;
后续的环境模块合并与 git 安全加固按路线图推进中。

## 版本历史

> 当前版本:**v7.1**。早期 v1–v3 在引入 Git 之前,为概述性追溯;v4 起依 Git 提交历史编写。
...
**v2.x —— 三命令与基础视觉(追溯)**
- **v2.0** —— 三命令格局成形:在 `gitmerge` 外新增 `gitsync`/`gitpush`(原子推送,后并入 `gitsync`)与 `gitstatus` 雏形,并加基础视觉(阶段标题/状态行/结果摘要)。

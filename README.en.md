...
![version](https://img.shields.io/badge/version-v7.5.1-blue) [![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)
...
> Current version: **v7.5.1**. Early v1–v3 predate Git tracking and are a summarized retrospective; from v4 on, the history follows the Git commit log.
>
...
- **v7.5.1** — Fix: a leftover `REBASE_HEAD` ref is **no longer mistaken for an in-progress rebase**. git keeps `REBASE_HEAD` after a rebase finishes/aborts (like `ORIG_HEAD`); it does not indicate an active rebase — the authoritative signal is the `rebase-merge`/`rebase-apply` **directory** (matching git's own `wt-status` check). Previously a stale `REBASE_HEAD` made `Test-WorktreeUsable` treat a clean branch as "untouchable", aborting `gitsync`/`gitmerge cross-all` on the current branch.
